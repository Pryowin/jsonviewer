...
/// double-clicks a Markdown file or uses "Open With" — SwiftUI's `App`
/// protocol has no scene-level hook for this, so it has to be handled here.
///
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func attach(model: MarkdownDocumentModel) {
        self.model = model
        if let pendingURL {
